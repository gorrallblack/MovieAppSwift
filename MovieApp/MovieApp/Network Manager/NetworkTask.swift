...
class NetworkTask<T: Decodable>: NSObject {
    ...
    private var cParameter: [String: Any]?
    var isAuthorization = NetworkUtil.Authorization.dontCare {
        didSet {
            if isAuthorization == .required {
                cHttpHeader?[NetworkUtil.AUTHORIZATION] = NetworkUtil.bearerAuth
            }
        }
    }
    ...
    ///
    /// - returns: Void.
    init(method: HTTPMethod = .get, parameter: [String: Any]? = nil, header: [String: String]? = nil) {
    ...
    func requestNetworkConnection(_ url: String) -> Promise<T> {
        ...
        return Promise { seal in
            AF.request(url, method: self.cHttpMethod, parameters: self.cParameter, encoding: URLEncoding.default, headers: self.cHttpHeader)
